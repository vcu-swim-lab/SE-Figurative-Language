...
# the dataset is big, the models regresses quicker
epochs = 6
delta = 0.1
batch_size = 128
...
print(model_flag, contrastive_flag)
...
train_df = pd.read_csv("../dataset/priority-small-train.csv")
val_df = pd.read_csv("../dataset/priority-small-test.csv")
...
for epoch in range(epochs):
    ...
    if val_loss > avg_test_loss:
        ...
        if contrastive_flag == 1:
            df.to_csv('results/bug_priority/bug_priority_' + model_name +'_pred_contrastive_' + str(epoch) + '_' + str(val_f1_score) + '.csv', index=False)
        else:
            df.to_csv('results/bug_priority/bug_priority_' + model_name +'_pred_simple_' + str(epoch) + '_' + str(val_f1_score) + '.csv', index=False)
