...
def compute_similarity(sentences, embedding_type, model, tokenizer):
    ...
    encoded_input_1 = tokenizer(sentence_1, padding=True, truncation=True, max_length=128, return_tensors='pt').to(device)
    encoded_input_2 = tokenizer(sentence_2, padding=True, truncation=True, max_length=128, return_tensors='pt').to(device)
    encoded_input_3 = tokenizer(sentence_3, padding=True, truncation=True, max_length=128, return_tensors='pt').to(device)
    ...
    sentence_embedding_1 = mean_pooling(model_output_1, encoded_input_1['attention_mask']).detach().cpu().clone()
    sentence_embedding_2 = mean_pooling(model_output_2, encoded_input_2['attention_mask']).detach().cpu().clone()
    sentence_embedding_3 = mean_pooling(model_output_3, encoded_input_3['attention_mask']).detach().cpu().clone()
